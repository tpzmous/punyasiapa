...
function addUrlButton(buttonName, buttonText, buttonUrl)
  dialogText = dialogText .. "add_url_button|comment|" .. buttonName .. "|noflags|" .. buttonUrl .. "|" .. buttonText .. "|0|0|add_spacer|small|\n"
end
log("`9BTW/CEME MIN 10DL-BGL AT `b[`2BTW`b]")
Sleep(400)
log([[
`9Script By Athar#0930
Do /proxy For Commands]])
function proxy()
  addLabelWithIcon("`2Proxy Gazzete", 758, "LABEL_BIG")
  addTextBox("Name : " .. GetLocal().name)
  addTextBox("User Id : " .. GetLocal().userid)
  addTextBox("`wScript Made By : `b[`wCPS COMMUNITY`b]")
  addSpacer("SPACER_SMALL")
  addUrlButton("CPS DISCORD COMMUNITY", "Join For More Info About Script CPS", "https://discord.gg/KAAxK2ZC")
  addTextBox("`bCREDIT `w: Vivi,Zenixa,Mildz")
  addTextBox("`bPARTNER `w: iHkaz#8706")
  addSpacer("SPACER_SMALL")
...
function options()
  addLabelWithIcon("`bOptions", 32, "LABEL_SMALL")
  addSpacer("SPACER_SMALL")
  addCheckbox("wrenchban", "`9Fast Pull", wrenchpull)
  addSpacer("SPACER_SMALL")
  endDialog("qwert", "Done", "Cancel")
  local var = {}
  var[0] = "OnDialogRequest"
  var[1] = dialogText
  var.netid = -1
...
function hook2(var)
  RunThread(function()
    if bethost == true and "OnConsoleMessage" == var[0] and var[1]:find("(%d+) Diamond Lock") then
      ...
      putih = ireng + ireng
      gabungan = math.percent(putih, taxcount)
      calculate = putih - gabungan
      dlwin = calculate % 100 % 100
      local vars = {}
      vars[0] = "OnTextOverlay"
      vars[1] = "`w[`cBET : `w" .. putih .. " `wDL] [`cTAX :  `w" .. gabungan .. " `cDiamond Locks]"
      vars.netid = -1
      SendVarlist(vars)
    end
  end)
  if "OnConsoleMessage" == var[0] and var[1]:find("(%d+) World Lock") then
    ireng1 = var[1]:match("(%d+) World Lock")
    putih = ireng + ireng
    gabungan = math.percent(putih, taxcount)
    calculate1 = putih - gabungan
    dlwin = calculate1 % 100 % 100
  end
end
function hook(var)
...
buttonClicked|bglconvert]])
  ...
  if wrenchpull and "OnDialogRequest" == var[0] and var[1]:find("add_button|trade|`wTrade") then
    tods = var[1]:match("netID|(%d+)")
    SendPacket(2, [[
action|dialog_return
dialog_name|popup
netID|]] .. tods .. [[
...
invitedWorld|0]])
  ...
  if 2 == type and packet:find("/set1") then
    log("`9This Commands `4Enable `9For Prem")
    return true
  end
...
bgl_count|]] .. daoa)
  ...
  if 2 == type and packet:find("/uncheck") then
    checkergems = false
    return true
  end
  ...
  if 2 == type and packet:find("/options") then
    log("`9This Commands `4Enable `9For Prem")
    return true
  end
  if 2 == type and packet:find("wrenchban|0") then
    wrenchpull = false
    log("`4Disable")
  end
  if 2 == type and packet:find("wrenchban|1") then
    wrenchpull = true
    log("`9Enable")
  end
